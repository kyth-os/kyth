...
# Vendored from Zeglius/titanoboa @ 7737f4748458252ac827dca14b3d6dd09298472a (build_iso.sh).
#
# The upstream GitHub Action runs this script inside its own container and
# accepts no volume mounts, so the KythOS iso.yaml fallback (see
# build_files/scripts/titanoboa-iso-wrapper.sh, mounted over /src/build_iso.sh
# with this file at /src/titanoboa-build_iso.sh) cannot be injected through
# the action. Vendored so CI runs Titanoboa directly via podman with the
# wrapper mounts, byte-identical to the pinned action revision apart from
# this header. If the pin in build-live-iso.sh moves, re-vendor this file
# from the new pin and update tests/test_titanoboa_iso_wrapper.py.
