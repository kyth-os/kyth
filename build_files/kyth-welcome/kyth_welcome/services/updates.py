"""Update-check helpers and optional Qt workers.

Pure API (registry re-exports, firmware command builders) imports without Qt.
Worker classes are defined only when Qt bindings are available.
"""
...
except ImportError:  # pragma: no cover - CI / headless without Qt
...
if _HAS_QT:
    ...
    class FlatpakCheckWorker(TrackedThread):
        """Query flatpak for pending system and user updates."""
        result = Signal(int)
    ...
    UpdateCheckWorker = None  # type: ignore[assignment,misc]
    FirmwareCheckWorker = None  # type: ignore[assignment,misc]
    ChangelogWorker = None  # type: ignore[assignment,misc]
    FlatpakCheckWorker = None  # type: ignore[assignment,misc]
