"""swappiness — swappiness.toml, 10 gaming vs 60."""
...
def config_path(path: Path|None=None) -> Path:
    ...
    if xdg and os.environ.get("KYTH_TEST_MODE")=="1": return Path(xdg)/"kyth"/"swappiness.toml"
    return DEFAULT_PATH
def load_swappiness(path: Path|None=None) -> dict[str,Any]:
    p=config_path(path)
    ...
    return {"profile":prof}
def save_swappiness(cfg: dict[str,Any], path: Path|None=None) -> Path:
    p=config_path(path); p.parent.mkdir(parents=True, exist_ok=True)
    prof=str(cfg.get("profile","balanced")).lower()
    if prof not in ("balanced","gaming"): prof="balanced"
    p.write_text(f"# Kyth swappiness — offline\nprofile = \"{prof}\"\n",encoding="utf-8"); return p
def generate_swappiness(cfg: dict[str,Any]|None=None, dest: Path|None=None) -> Path|None:
    if cfg is None: cfg=load_swappiness()
