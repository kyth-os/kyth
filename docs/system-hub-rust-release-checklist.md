...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 91 of the 94 tunable entries;
the remaining 3 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 3 tunable entries whose writers are module-specific,
