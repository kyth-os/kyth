...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 78 of the 94 tunable entries;
the remaining 16 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 16 tunable entries whose writers are module-specific,
