...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 87 of the 94 tunable entries;
the remaining 7 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 7 tunable entries whose writers are module-specific,
