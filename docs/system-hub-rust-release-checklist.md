...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 85 of the 94 tunable entries;
the remaining 9 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 9 tunable entries whose writers are module-specific,
