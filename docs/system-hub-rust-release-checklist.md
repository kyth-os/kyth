...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 76 of the 94 tunable entries;
the remaining 18 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 18 tunable entries whose writers are module-specific,
