...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 89 of the 94 tunable entries;
the remaining 5 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 5 tunable entries whose writers are module-specific,
