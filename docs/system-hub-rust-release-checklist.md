...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 70 of the 94 tunable entries;
the remaining 24 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 24 tunable entries whose writers are module-specific,
