...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 67 of the 94 tunable entries;
the remaining 27 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 27 tunable entries whose writers are module-specific,
