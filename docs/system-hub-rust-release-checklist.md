...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 63 of the 94 tunable entries;
the remaining 31 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 31 tunable entries whose writers are module-specific,
