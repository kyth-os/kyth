...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 52 of the 94 tunable entries;
the remaining 42 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 42 tunable entries whose writers are module-specific,
