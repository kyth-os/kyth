...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 75 of the 94 tunable entries;
the remaining 19 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 19 tunable entries whose writers are module-specific,
