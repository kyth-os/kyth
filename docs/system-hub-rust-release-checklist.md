...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 69 of the 94 tunable entries;
the remaining 25 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 25 tunable entries whose writers are module-specific,
