...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 90 of the 94 tunable entries;
the remaining 4 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 4 tunable entries whose writers are module-specific,
