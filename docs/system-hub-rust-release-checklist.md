...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 92 of the 94 tunable entries;
the remaining 2 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 2 tunable entries whose writers are module-specific,
