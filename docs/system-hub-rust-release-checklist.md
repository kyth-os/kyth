...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 81 of the 94 tunable entries;
the remaining 13 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 13 tunable entries whose writers are module-specific,
