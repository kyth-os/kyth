...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 83 of the 94 tunable entries;
the remaining 11 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 11 tunable entries whose writers are module-specific,
