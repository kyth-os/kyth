...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 73 of the 94 tunable entries;
the remaining 21 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 21 tunable entries whose writers are module-specific,
