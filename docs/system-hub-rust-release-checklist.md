...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 66 of the 94 tunable entries;
the remaining 28 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 28 tunable entries whose writers are module-specific,
