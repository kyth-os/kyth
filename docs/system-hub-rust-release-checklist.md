...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 79 of the 94 tunable entries;
the remaining 15 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 15 tunable entries whose writers are module-specific,
