...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 84 of the 94 tunable entries;
the remaining 10 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 10 tunable entries whose writers are module-specific,
