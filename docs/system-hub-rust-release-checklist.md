...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 74 of the 94 tunable entries;
the remaining 20 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 20 tunable entries whose writers are module-specific,
