...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 72 of the 94 tunable entries;
the remaining 22 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 22 tunable entries whose writers are module-specific,
