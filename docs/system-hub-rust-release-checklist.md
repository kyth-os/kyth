...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 86 of the 94 tunable entries;
the remaining 8 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 8 tunable entries whose writers are module-specific,
