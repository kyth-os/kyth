...
or autostarted.
The native `kyth-tunable-rs` dispatcher now owns 88 of the 94 tunable entries;
the remaining 6 entries continue through the explicit compatibility fallback.
...
- [ ] Port the remaining 6 tunable entries whose writers are module-specific,
