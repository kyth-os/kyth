...
| Legacy `Update` | `Updates.tsx` | `updates.rs`, `dashboard.rs`, `liveData.ts` | The Updates page owns one read/check/stage/restart workflow; update jobs expose explicit running/complete/failed state and rollback/restart guidance is data-backed | Extended watcher parity and service-level validation remain tracked in Phase 5 |
