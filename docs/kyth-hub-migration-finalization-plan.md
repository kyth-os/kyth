...
points, 49 sysctl-backed tunable entries, and 17 module-specific tunable entries
are now native Rust/non-Python. The remaining 28 module-specific tunables still
...
- [x] Port the 49 sysctl-backed entries and 17 module-specific entries of the
...
- [x] Port `distrobox-cache`, `flatpak-prefetch`, and `flatpak-trim` writers to
      native `kyth-tunable-rs` (2026-09-03); generated unit/timer files remain
      reversible and service activation stays caller-owned.
- [ ] Port the remaining 28 module-specific tunable writers, remove the
...
| P2 | Port remaining indirect recipe executors (`kyth-tunable`) and remove its compatibility module. | 7 (49 sysctl and 17 module-specific entries complete 2026-09-03; 28 fallback entries remain) |
