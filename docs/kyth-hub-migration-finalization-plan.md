...
points, 49 sysctl-backed tunable entries, and 18 module-specific tunable entries
are now native Rust/non-Python. The remaining 27 module-specific tunables still
...
- [x] Port the 49 sysctl-backed entries and 18 module-specific entries of the
...
- [x] Port `distrobox-cache`, `flatpak-prefetch`, `flatpak-trim`, and `readahead` writers to
      native `kyth-tunable-rs` (2026-09-03); generated unit/timer files remain
      reversible and service activation stays caller-owned.
- [ ] Port the remaining 27 module-specific tunable writers, remove the
...
| P2 | Port remaining indirect recipe executors (`kyth-tunable`) and remove its compatibility module. | 7 (49 sysctl and 18 module-specific entries complete 2026-09-03; 27 fallback entries remain) |
