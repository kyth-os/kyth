...
- [x] qualification and the acceptance/report execution path — packaged
  ...
  fixture.
- [x] perf_gate and its validation caller — `kyth-perf-gate-rs` owns timing,
  ledger comparison, baseline recording, and failure semantics. The Python
  script is only a compatibility delegator.
...
accounted for, with 106 explicit dispatcher routes, 93 native fallback routes,
...
The source-derived ledger now reports all 202 recipes accounted for, with 106
explicit dispatcher routes, 93 native fallback routes, 3 explicit retirements,
