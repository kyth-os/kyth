...
|---|---|---|
| `/api/start` | Installation state plus confirmation flags: `disk`, `hostname`, `timezone`, `username`, `password`, `locale`, `keymap`, `kernel`, `install_mode`, optional `target_partition`, `resize_partition`, `resize_gib`, `free_region_start`, `free_region_end`, `encryption` (`none` default, `tpm2` for `to-disk` only; anything else fails closed), `tpm_recovery_ack` (required `true` for `tpm2`: TPM-bound LUKS has no passphrase, so without an escrowed recovery key a board swap, TPM reset, or PCR change destroys access), `confirm_backup`, `confirm_erase`, `confirm_current` | `{started: true}` after validation and worker launch. A running install is 409. |
