...
export function MoveInOverview() {
  ...
  return <section className="move-in-overview" aria-label="Move In overview">
    ...
    <div className="move-in-card-grid">
      ...
      {/* Every sibling card's status dot reflects the actual data (drive
          count, provider count, VPN connected) — this one showed green
          "OK" the instant network info loaded, even with zero shares
          mounted or saved, contradicting its own "No shares mounted" text
          right next to it. */}
      <MoveCard icon="▰" label="Network shares" value={shareValue} detail={networkKnown ? `${readings.network?.smbMounts ?? 0} currently mounted · ${readings.shares?.length ?? 0} saved` : "Share discovery is not available yet."} status={shareCount === null ? null : shareCount > 0} />
