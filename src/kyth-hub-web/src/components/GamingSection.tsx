...
function GamingToolGrid({ busy, run }: { busy: string | null; run: SectionRun }) {
  ...
  return (
    <div style={{ marginTop: 22 }}>
      ...
      {tools.length === 0 && <p className="card-copy" style={{ fontSize: 12, marginTop: 4 }}>The tool catalog did not load — use Refresh below to retry.</p>}
  ...
  return (
    <div style={{ marginTop: 22 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
        ...
        <button disabled={busy !== null} onClick={() => run("gt-install-dev.vencord.Vesktop", "Installing Vesktop…", () => installGamingTool("dev.vencord.Vesktop"))} style={gamingBtnStyle}>Install Vesktop (Discord voice)</button>
...
// "Fix My Game" card.
function FixMyGame({ busy, run, steamPlay }: { busy: string | null; run: SectionRun; steamPlay: SteamPlayStatus | null }) {
  return (
    <div style={{ marginTop: 22 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
        <button disabled={busy !== null || (steamPlay !== null && !steamPlay.steam_present)} title={steamPlay && !steamPlay.steam_present ? "Steam is not installed yet" : undefined} onClick={() => run("open-compatdata", "Opening compatdata…", () => openGameFolder("compatdata"))} style={gamingBtnStyle}>Open Steam compatdata</button>
        <button disabled={busy !== null || (steamPlay !== null && !steamPlay.steam_present)} title={steamPlay && !steamPlay.steam_present ? "Steam is not installed yet" : undefined} onClick={() => run("open-shadercache", "Opening shadercache…", () => openGameFolder("shadercache"))} style={gamingBtnStyle}>Open shadercache</button>
      </div>
...
export function GamingSection({ section }: { section: HubSection }) {
  ...
  const [steamPlay, setSteamPlay] = useState<SteamPlayStatus | null>(null);
    ...
    <LiveSectionCard section={section} live={live}>
      {audit ? (
        <div style={{ marginTop: 20 }}>
          ...
          <p className="card-copy" style={{ fontSize: 12, marginTop: 4 }}>The gaming profile favors frame latency over power savings (CPU scheduling, I/O priority, compositor latency). Balanced is quieter on battery and fans — switch back any time.</p>
      ...
      <div style={{ marginTop: 20, borderTop: "1px solid var(--hairline)", paddingTop: 16 }}>
        ...
        {steamPlay && steamPlay.steam_present && (
          <div style={{ marginTop: 20 }}>
            <p className="card-copy" style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 0.6 }}>Steam Play defaults</p>
            <span className={`pill ${steamPlay.mapping_present ? "pill-ok" : "pill-dim"}`}>{steamPlay.mapping_present ? "enabled for all titles" : "not set"}</span>
            {!steamPlay.mapping_present && <p className="card-copy" style={{ fontSize: 12, marginTop: 8 }}>{steamPlay.detail}</p>}
          </div>
        )}
        <div style={{ marginTop: 20 }}>
          ...
          {proton.length > 0 ? proton.map((result) => <p key={result.app_id} className="card-copy" style={{ fontSize: 12, marginTop: 8 }}>{result.app_id}: {result.detail}</p>) : gameId && <p className="card-copy" style={{ fontSize: 12, marginTop: 8 }}>Enter one or more Steam app IDs and look up current ProtonDB reports. Know the name but not the ID? <a href={`https://www.protondb.com/search?q=${encodeURIComponent(gameId)}`} target="_blank" rel="noreferrer">Search ProtonDB by name</a>, then paste the app ID here.</p>}
        </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>
          <RecipeButton recipe="gaming-mode" label={audit && String(audit.master ?? "") === "gaming" ? "Gaming profile on ✓" : "Switch to gaming profile"} busy={busy} run={run} />
        ...
        <FixMyGame busy={busy} run={run} steamPlay={steamPlay} />
        <OverlaysCard />
