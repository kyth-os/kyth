...
| Move In | Move Files, Cloud Storage, Network Shares, VPN | `MoveIn.tsx` → 4 sections, all with actions | Move Files: `ntfs-drives` cache + live `ntfs_devices` rescan. Cloud: `network-summary` + `cloud_oauth_status` + `rclone_oauth_command`. Shares: `smb_browse`/temporary mount command plus persisted add/remove through the existing root `kyth-network-share` helper; credentials stay off the user config. VPN: `network-summary` + live `network_identity` refresh and a fixed launch path to the existing full Python profile/SAML client. All three network sections escalate to `network_identity` on Refresh |
...
Python `page_software.py` 7 mixins (Starter Packs, Flatpak Store, AppImages, Installed, Developer, Security, Creator) with `software_catalogs.py` (`STARTER_PACKS`, `SEC_BOX`, `FAMILAR_APPS`). React `AppStoreSection` covers Flatpak counts, starter packs with selectable apps, the familiar-app chooser, debounced Flathub search/install, AppImage discovery/import/launch, installed Flatpak removal, and recipe-backed developer tools. Install actions poll the Rust job bridge and refresh the inventory after completion. **Security tab is the real remaining gap here**: Python's `page_software_security_kali.py` (339 lines) drives a full root distrobox Kali container lifecycle — tiered create (headless/default/everything) with streamed progress, terminal launch, GUI-app export with a live exported-count, and remove — plus `page_software_security_hosttools.py` (192 lines), a grid of host-side Flatpak security tools. `AppStoreSection.tsx` only offers a static `export-kali-apps` recipe button and a copyable `ujust setup-kali-box` command; the interactive create/enter/export/remove flow and the host-tools grid are not ported.

### 4. Guardian repairs — DONE (command table + eligibility gate ported, recipe set expanded beyond Python's original)
`guardian_execute_recipe` used to hand the recipe id to `just_run`. Guardian ids are dotted (`audio.restart`) and are not just recipes, so nothing ever ran — and the spawn still succeeded, so the Hub reported every repair as launched, advisory notifications included. `guardian.rs` now carries each recipe's `command` argv from `guardian.py`, ports the user-initiated gate, implements all ten `guardian_actions.py:ACTION_EXECUTORS` with bounded argv calls, verifies results, enforces cooldowns, and records Hub executions in Guardian history — plus eleven Rust-only recipes (`bluetooth.restart`, `disk.review`, `flatpak.refresh-metadata`, `flatpak.repair-user`, `memory.pressure-relief`, `network.restart-user`, `plasma.restart-user`, `storage.smart-warn`, `thermal.notify`, `update.review-health`) with no Python equivalent. `GuardianSection` only offers "Run fix" for a runnable risk. The live Guardian sweep and service state writer remain Python-owned (`kyth-guardian.service`); that's a backing-service gap, not a Hub UI one — see item 5.

### 5. kyth_shared → kyth-shared-rs coverage
Python `src/kyth_shared/kyth_shared` `251` files / `≈1567` defs vs Rust `src/kyth-shared-rs/src` `176` files (`151` under `system::`) — roughly 70% file-count coverage, up from the `≈15%` this line quoted for a long time; check `src/kyth-shared-rs/MIGRATION.md`'s "What's ported so far" table for the current module list rather than trusting a static count here, since it drifts every time a module lands. Coverage is skewed toward read-only inventory/parsers by design (`MIGRATION.md`'s "read-only first" rule); installer partitioning, SELinux, VPN connect, `zypp`/`dnf`, and `collect_snapshot` stay Python-owned deliberately. The Hub's explicit Guardian repair path is ported with the same eligibility, cooldown, verification, and fixed-command policy as Python; the live Guardian sweep and probe-service collector (`kyth-probe.service`/`kyth-guardian.service`) remain Python-owned — that's the actual blocker to ever deleting `build_files/kyth-welcome/`, independent of how complete the Hub UI itself is. Parity for the Hub UI does not require 100% of `kyth_shared` — only the UI-facing reads plus the small set of explicitly exposed mutating actions.
