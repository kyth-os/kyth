...
- **A recipe is never reported as complete before it finishes.** `just_run` starts a captured background job, while upgrade, rollback, channel switching, and staged-update activation use the native Rust update job bridge directly. The Tauri shell waits for the process result, keeps a concise stdout/stderr summary, and the Hub renders running, complete, or failed status inline. KDE's graphical askpass helper handles sudo authentication when needed, so the user never has to hunt for a newly opened terminal window.
