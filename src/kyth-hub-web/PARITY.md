...
| Updates | Updates | `Updates.tsx` → dedicated update page | `update_status`/`pending_updates_summary` on mount, `collect_availability` on demand; one page-level action surface owns check, stage, restart, app updates, and rollback |
...
- **Cheap reads on mount, expensive reads on a button.** `mokutil` (~seconds), `fwupd` (20s timeout), `collect_availability` (Hub-side 95s deadline), `ipp_discover`/`smb_browse` (network) and the live driver scan all sit behind an explicit button so switching tabs never stalls.
...
- **One job per domain; a second launch into an occupied slot is rejected.** `services/liveData.ts` tracks at most one in-flight job per domain (`guardian`, `privileged`, `hub-action`, `update`, `job`, `install`, `security`, `gaming`); `trackJob` and the shared domain poller both throw an already-running error for a different job id while the first keeps the slot, so a section's Cancel button always reaches the real backend job.
- **In-flight jobs persist across reloads and reattach on init.** Tracked ids are written to `localStorage` (`kyth-hub:inflight-jobs`) with a timestamp; on module init `reattachInFlightJobs` restores them (accepting the legacy plain-string shape), drops entries older than the 2h `REATTACHED_JOB_TTL_MS` bound and anything failing the `<prefix>-<nanos>` id allowlist, and `validateReattachedJobs` probes each reattached id once against its domain's status command — unknown jobs are dropped, probe failures keep the slot. A `storage` event listener keeps tracking cross-tab single-flight: a Cancel in one tab stops the other tab's poller on its next tick.
- **Domains share one poller with backoff.** Concurrent waiters for the same job share a single promise and probe cadence; probes run every 500ms for the first 30s, then every 2s (slower-based pollers such as security/gaming at 3s keep their cadence). Five consecutive empty probes are lost-contact; the per-domain probe limit is the timeout.
- **The update-availability check has a Hub-side 95s timeout.** `checkForUpdates` races `collect_availability` against a 95s timer so the button always settles with an actionable error ("your current system has not changed") instead of spinning past a dead mirror.
