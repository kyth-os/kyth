...
def generate_memory_tune(cfg: dict[str, Any] | None = None, dest: Path | None = None, mem_kb: int | None = None) -> Path:
    ...
    if dest == DEFAULT_CONF:
        try:
            ...
            # Same numbers as the drop-in above, in a format kyth-zram-swap
            # can source with plain bash instead of parsing "min(ram * N, M)".
            runtime_content = (
            ...
            DEFAULT_ZRAM_RUNTIME_ENV.parent.mkdir(parents=True, exist_ok=True)
            tmp3 = DEFAULT_ZRAM_RUNTIME_ENV.with_suffix(".tmp")
            tmp3.write_text(runtime_content, encoding="utf-8")
            tmp3.replace(DEFAULT_ZRAM_RUNTIME_ENV)
