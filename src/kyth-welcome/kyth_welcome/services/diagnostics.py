...
def _system_hub_probe() -> HardwareProbe:
    ...
    if not app_ok:
        return HardwareProbe(
            "Kyth Hub", "err",
            "Kyth Hub executable is missing or not runnable.",
            "\n".join(details),
    ...
    if not launcher_ok or not desktop_uses_launcher or not autostart_uses_launcher:
        return HardwareProbe(
            "Kyth Hub", "warn",
            "Kyth Hub is installed, but the launcher diagnostics wrapper is not fully active.",
            "\n".join(details),
    ...
    if _system_hub_log_has_self_error(log_tail):
        return HardwareProbe(
            "Kyth Hub", "warn",
            "Recent Kyth Hub launch log contains an error.",
            "\n".join(details),
    ...
    return HardwareProbe(
        "Kyth Hub", "ok",
        "Kyth Hub launcher and diagnostics wrapper are installed.",
        "\n".join(details),
    )
 # _system_hub_probe
