...
class RepairPage(Page, _QuickFixMixin, _AssistMixin, _ResetMixin):
    ...
    def _build_guardian_link_card(self) -> None:
        """Short link to the dedicated Guardian page (System → Guardian)."""
        card, layout = _make_card("card-accent-ok")
        title = QLabel("Kyth Guardian — self-healing (moved)")
        title.setObjectName("card-title")
        layout.addWidget(title)
        body = QLabel(
            "Guardian now has its own page: System → Guardian. Open it for live health, "
            "automatic safe fixes, full history, recipe catalogue, and the optional local AI model. "
            "This Repair page keeps rollback, reset, and recovery."
        )
        ...
        row = QHBoxLayout()
        row.setSpacing(8)
        open_btn = QPushButton("Open Guardian")
        open_btn.setObjectName("primary")
        open_btn.clicked.connect(lambda _=False: self._navigate("Guardian"))
