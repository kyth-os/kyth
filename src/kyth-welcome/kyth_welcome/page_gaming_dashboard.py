...
class _DashboardMixin:
    ...
    def _make_gaming_hud_grid(self) -> QWidget:
        ...
        title4 = QLabel("QUICK PERFORMANCE — Hub Guardian")
        title4.setObjectName("hud-title")
        layout4.addWidget(title4)
        perf_hint = QLabel("Power profile + display VRR + controller stack — all in Hub > This PC > Guardian. One-click Fix My Gaming uses same gated runner as Starter Packs.")
    ...
    def _update_perf_hub_labels(self):
        ...
        def _apply(_key: str, comps: object) -> None:
            ...
            if hasattr(self, "_hud_perf_profile"):
                if "power" in comps:
                    self._hud_perf_profile.setText("Power: drift — Hub > Guardian will reset profile")
                else:
                    self._hud_perf_profile.setText("Power: ok (Hub > Guardian fixes stuck profile)")
