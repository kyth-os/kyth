...
| `system::hdr_store`, `hdr_per_game` | HDR preserve preference and native hdr-store dispatch plus per-game peak/ITM config models | KWin/display mutation and driver probing remain outside the shared crate. |
...
| `system::preference_presets` | Fonts, locale, OOMD, immutable `/etc` overlay, and native Steam deadzone/SELinux gaming config dispatch | Desktop/system policy application and overlay activation remain outside Rust. |
...
| `system::tunable_registry` | Declarative tunable catalog loading, name normalization, and safe lookup/listing used by both dispatchers | Dynamic dispatch for the remaining compatibility entries and their module-specific application remain Python-owned. |
