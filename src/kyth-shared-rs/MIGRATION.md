...
| Native diagnostic entry points | `kyth-post-update-check`, `kyth-firstboot-app-status`, `kyth-steam-game-export`, and `kyth-hub-desktop-entries` are Rust binaries built with this crate; they replace the installed Python helpers and packaging generator | Runtime/user acceptance and provider-specific qualification remain separate release gates. |
