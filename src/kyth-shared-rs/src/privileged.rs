...
fn share_text<'a>(payload: &'a Map<String, Value>, field: &str, allow_empty: bool, maximum: usize) -> Result<&'a str, String> {
    let value = payload
        .get(field)
...
fn network_share_stdin(request: &Value, operation: &str, uid: u32, gid: u32) -> Result<Vec<u8>, String> {
    let payload = request
        .get("payload")
        .and_then(Value::as_object)
        .ok_or_else(|| "network share payload must be an object".to_string())?;
    if serde_json::to_vec(payload).map_err(|_| "network share payload is invalid".to_string())?.len() > MAX_SHARE_PAYLOAD {
        return Err("network share payload is too large".to_string());
    }
    let name = share_text(payload, "name", false, 64)?;
    if !Regex::new(r"^[A-Za-z0-9_-]{1,64}$").expect("share name regex").is_match(name) {
    ...
    if operation == "network_share_add" {
        ...
        if !Regex::new(r"^[A-Za-z0-9._:-]{1,253}$").expect("share host regex").is_match(server) {
        ...
        output.insert("auto_mount".into(), json!(payload.get("auto_mount").and_then(Value::as_bool).unwrap_or(false)));
        output.insert("mount_now".into(), json!(payload.get("mount_now").and_then(Value::as_bool).unwrap_or(false)));
    ...
    serde_json::to_vec(&Value::Object(output)).map_err(|_| "network share payload is invalid".to_string())
}

fn valid_flatpak_id(value: &str) -> bool {
    Regex::new(r"^[A-Za-z0-9]+(?:[.-][A-Za-z0-9_]+)+$").expect("Flatpak id regex").is_match(value)
}

fn valid_block_device(value: &str) -> bool {
    Regex::new(r"^/dev/(sd[a-z][0-9]*|nvme[0-9]+n[0-9]+p?[0-9]*|vd[a-z][0-9]*|mmcblk[0-9]+p?[0-9]*)$")
...
fn validate_request(request: &Value, uid: u32, gid: u32) -> Result<ExecSpec, String> {
    let object = request.as_object().ok_or_else(|| "request must be an object".to_string())?;
    let operation = object.get("operation").and_then(Value::as_str).unwrap_or("");
    let (argv, stdin) = match operation {
        "flatpak_uninstall" => {
            ...
            (vec!["/usr/bin/flatpak", "uninstall", "--system", "-y", app_id], None)
        }
        ...
        "bitlocker_unlock" => {
            ...
            (vec!["/usr/bin/udisksctl", "unlock", "-b", device, "--key-file", "/dev/stdin"], Some(key.as_bytes().to_vec()))
        }
        "network_share_add" | "network_share_remove" => (
            vec!["/usr/libexec/kyth-network-share", if operation == "network_share_add" { "add" } else { "remove" }],
...
fn display_detail(stdout: &[u8], stderr: &[u8], status: std::process::ExitStatus) -> String {
    let output = if stdout.iter().any(|byte| !byte.is_ascii_whitespace()) { stdout } else { stderr };
    let detail = String::from_utf8_lossy(output).trim().chars().take(400).collect::<String>();
    if !detail.is_empty() {
        detail
...
fn run_operation(spec: ExecSpec) -> Result<String, String> {
    ...
    command.args(&spec.argv[1..]).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped());
    let mut child = command.spawn().map_err(|error| format!("could not start privileged operation: {error}"))?;
    if let Some(input) = spec.stdin {
        child.stdin.take().expect("piped stdin").write_all(&input).map_err(|error| format!("could not provide privileged input: {error}"))?;
    }
    let stdout = child.stdout.take().expect("piped stdout");
    let stderr = child.stderr.take().expect("piped stderr");
    let stdout_thread = thread::spawn(move || {
        let mut bytes = Vec::new();
        let _ = BufReader::new(stdout).take(64 * 1024).read_to_end(&mut bytes);
        bytes
    });
    let stderr_thread = thread::spawn(move || {
        let mut bytes = Vec::new();
        let _ = BufReader::new(stderr).take(64 * 1024).read_to_end(&mut bytes);
        bytes
    });
    let deadline = Instant::now() + OPERATION_TIMEOUT;
    let status = loop {
        match child.try_wait().map_err(|error| format!("could not wait for privileged operation: {error}"))? {
    ...
    if status.success() { Ok(detail) } else { Err(detail) }
}
...
fn audit(uid: u32, operation: &str, ok: bool, detail: &str) {
    let result = (|| -> io::Result<()> {
        ...
        writeln!(file, "{timestamp} uid={uid} operation={} ok={} detail={}", sanitize(operation), u8::from(ok), sanitize(detail))
    })();
    if result.is_err() {
...
fn wheel_gid() -> Result<u32, String> {
    let groups = fs::read_to_string("/etc/group").map_err(|error| format!("could not read groups: {error}"))?;
    groups.lines().find_map(|line| {
        let mut fields = line.split(':');
        (fields.next() == Some("wheel")).then(|| fields.next()).flatten()?.parse().ok()
...
fn peer_credentials(stream: &UnixStream) -> io::Result<(u32, u32, u32)> {
    ...
    let result = unsafe {
        libc::getsockopt(stream.as_raw_fd(), libc::SOL_SOCKET, libc::SO_PEERCRED, &mut credentials as *mut _ as *mut libc::c_void, &mut length)
    };
...
fn peer_allowed(stream: &UnixStream) -> Result<(u32, u32, u32), String> {
    ...
    let supplementary = status.lines().find_map(|line| line.strip_prefix("Groups:")).map(|line| line.split_whitespace().filter_map(|value| value.parse::<u32>().ok()).any(|group| group == wheel)).unwrap_or(false);
    if !supplementary { return Err("caller is not root or a wheel member".to_string()); }
    Ok((pid, uid, gid))
}
...
pub fn redact_request_detail(request: &Value, detail: &str) -> String {
    ...
    for secret in secrets.into_iter().flatten().filter(|value| !value.is_empty()) {
    ...
    loop {
        ...
        match reader.read_line(&mut line) {
            ...
            Ok(_) => {
                ...
                let operation = request.as_ref().ok().and_then(|value| value.get("operation")).and_then(Value::as_str).unwrap_or("invalid").to_string();
                ...
                let detail = request
                    .as_ref()
                    .map_or_else(|_| crate::system::process::redact_sensitive_text(&detail), |value| redact_request_detail(value, &detail));
                audit(uid, &operation, ok, &detail);
                if writer.write_all(&response(ok, &detail)).is_err() { break; }
                let _ = writer.flush();
            }
            Err(error) => {
...
pub fn serve() -> Result<(), String> {
    ...
    let wheel = wheel_gid()?;
    fs::create_dir_all("/run/kyth").map_err(|error| format!("could not create runtime directory: {error}"))?;
    match fs::symlink_metadata(SOCKET) {
        Ok(metadata) if metadata.file_type().is_socket() => fs::remove_file(SOCKET).map_err(|error| error.to_string())?,
        Ok(_) => return Err("privileged socket path exists and is not a socket".to_string()),
        Err(error) if error.kind() == io::ErrorKind::NotFound => {}
        Err(error) => return Err(error.to_string()),
    }
    let listener = UnixListener::bind(SOCKET).map_err(|error| format!("could not bind privileged socket: {error}"))?;
    let fd = listener.as_raw_fd();
    let chown_result = unsafe { libc::fchown(fd, 0, wheel) };
    if chown_result != 0 { return Err(format!("could not set socket owner: {}", io::Error::last_os_error())); }
    let chmod_result = unsafe { libc::fchmod(fd, 0o660) };
    if chmod_result != 0 { return Err(format!("could not set socket mode: {}", io::Error::last_os_error())); }
...
mod tests {
    ...
    fn only_fixed_operations_are_constructed() {
        ...
        let request = validate_request(&json!({"operation":"kernel_switch","flavor":"cachy"}), 1000, 1000).unwrap();
        assert_eq!(request.argv, ["/usr/bin/ujust", "switch-kernel", "cachy"]);
        assert!(validate_request(&json!({"operation":"flatpak_uninstall","app_id":"org.example.App"}), 1000, 1000).is_ok());
        assert!(validate_request(&json!({"operation":"flatpak_uninstall","app_id":"_org.example"}), 1000, 1000).is_err());
    }

    #[test]
    fn bitlocker_key_is_stdin_only_and_validation_never_echoes_it() {
        let error = validate_request(&json!({"operation":"bitlocker_unlock","device":"/tmp/disk","key":"secret-key"}), 1000, 1000).unwrap_err();
        assert_eq!(error, "invalid block device");
        let error = validate_request(&json!({"operation":"bitlocker_unlock","device":"/dev/sda1","key":"short"}), 1000, 1000).unwrap_err();
        assert_eq!(error, "invalid BitLocker key");
        let request = validate_request(&json!({"operation":"bitlocker_unlock","device":"/dev/sda1","key":"12345678"}), 1000, 1000).unwrap();
    ...
    fn network_share_uses_peer_identity_and_drops_untrusted_fields() {
        ...
        assert!(validate_request(&json!({"operation":"network_share_add","payload":{"name":"bad/name"}}), 1000, 1000).is_err());
    }
