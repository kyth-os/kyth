...
fn patterns() -> &'static [(&'static str, Regex)] {
    ...
    PATTERNS.get_or_init(|| vec![
        ...
        ("age key", Regex::new(r"AGE-SECRET-KEY-1[0-9A-Z]{58}").unwrap()),
        ("cosign private key", Regex::new(r"-----BEGIN ENCRYPTED COSIGN PRIVATE KEY-----").unwrap()),
        ("GitHub token", Regex::new(r"\b(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{36,}\b").unwrap()),
        ("GitHub fine-grained token", Regex::new(r"\bgithub_pat_[A-Za-z0-9_]{80,}\b").unwrap()),
        ("AWS access key", Regex::new(r"\b(?:AKIA|ASIA)[0-9A-Z]{16}\b").unwrap()),
        ("Slack token", Regex::new(r"\bxox[baprs]-[A-Za-z0-9-]{20,}\b").unwrap()),
        ("generic high-entropy secret", RegexBuilder::new(r"\b[A-Za-z0-9+/]{40,}={0,2}\b.*\b(?:secret|token|key)\b").case_insensitive(true).build().unwrap()),
    ])
}

pub fn is_binary_suffix(path: impl AsRef<Path>) -> bool {
    matches!(path.as_ref().extension().and_then(|extension| extension.to_str()).map(str::to_ascii_lowercase).as_deref(), Some("cer" | "png" | "jpg" | "jpeg" | "webp" | "ico"))
}

pub fn scan_text(text: &str) -> Vec<SecretFinding> {
    patterns().iter().filter_map(|(kind, pattern)| pattern.is_match(text).then_some(SecretFinding { kind })).collect()
}
...
mod tests {
    ...
    fn catches_high_confidence_secret_formats() {
        ...
        assert!(findings.iter().any(|finding| finding.kind == "GitHub token"));
        assert!(scan_text("-----BEGIN PRIVATE KEY-----").iter().any(|finding| finding.kind == "private key block"));
    }

    #[test]
    fn binary_suffixes_are_excluded_by_file_helper() {
        assert!(is_binary_suffix("logo.PNG"));
        assert!(scan_file_text("logo.png", "ghp_abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJ").is_empty());
