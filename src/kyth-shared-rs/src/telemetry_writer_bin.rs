...
fn settings() -> (u64, u64) {
    ...
    let Ok(raw) = std::fs::read_to_string(path) else { return (10, 15); };
    let Ok(value) = raw.parse::<toml::Value>() else { return (10, 15); };
    let section = value.get("telemetry").unwrap_or(&value);
    let interval = section.get("scan_interval").and_then(toml::Value::as_integer).unwrap_or(10).max(1) as u64;
    let min_age = section.get("min_file_age").and_then(toml::Value::as_integer).unwrap_or(15).max(0) as u64;
    (interval, min_age)
}
...
fn main() -> std::process::ExitCode {
    ...
    if let Err(error) = kyth_shared::system::telemetry_writer::ensure_mangohud_output(&home, &sessions) {
    ...
    loop {
        match kyth_shared::system::telemetry_writer::scan_directory(&mut conn, &sessions, min_age, std::time::SystemTime::now(), kyth_shared::system::telemetry_writer::current_unix_time()) {
