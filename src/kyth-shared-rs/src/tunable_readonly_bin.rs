...
//!
//! The mutation-capable dispatcher lives in `kyth-tunable-rs`; this binary is
//! retained for callers that only need registry discovery and status output.
...
fn main() {
    ...
    let active = if spec.kind == "sysctl" {
        if std::path::Path::new(&drop_in).is_file() { "configured" } else { "absent" }
    } else {
        "unknown"
    };
...
mod tests {
    ...
    fn registry_has_expected_complete_split() {
        ...
        assert_eq!(specs.iter().filter(|spec| spec.kind == "sysctl").count(), 49);
