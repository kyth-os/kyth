...
const SESSION_CONF: &str = "[General]\nDefaultSession=plasma.desktop\n\n[Autologin]\nSession=plasma.desktop\n";

fn arg_path(args: &[String], name: &str, default: &str) -> PathBuf {
    args.windows(2).find(|pair| pair[0] == name).map(|pair| PathBuf::from(&pair[1])).unwrap_or_else(|| PathBuf::from(default))
}
fn arg_value(args: &[String], name: &str) -> Option<String> {
    args.windows(2).find(|pair| pair[0] == name).map(|pair| pair[1].clone())
}

fn warn(error: impl std::fmt::Display) { eprintln!("Warning: {error}"); }

fn rewrite_file(path: &Path) {
    let Ok(text) = fs::read_to_string(path) else { return };
    let (rewritten, changed) = kyth_shared::system::wayland::rewrite_session_key(&text, "Session");
    if changed { if let Err(error) = fs::write(path, rewritten) { warn(format!("could not migrate {path:?}: {error}")); } }
}
...
fn migrate_homes(root: &Path) {
    ...
    for entry in entries.flatten() {
        let home = entry.path();
        if home.is_dir() { rewrite_file(&home.join(".dmrc")); }
    }
}
...
fn main() {
    ...
    let cmdline = arg_value(&args, "--cmdline").or_else(|| fs::read_to_string("/proc/cmdline").ok()).unwrap_or_default();

    if let Err(error) = fs::create_dir_all(&conf_dir) { warn(format!("could not create {conf_dir:?}: {error}")); return; }
    if let Err(error) = fs::write(conf_dir.join("11-kyth-session.conf"), SESSION_CONF) { warn(format!("could not write session configuration: {error}")); }
    rewrite_file(&state);
    ...
    let body = if kyth_shared::system::wayland::needs_software_compose(&dri, Some(&cmdline)) {
        kyth_shared::system::wayland::software_compose_env().into_iter().map(|(key, value)| format!("{key}={value}\n")).collect()
    } else { String::new() };
    if let Err(error) = fs::write(&env_file, body) { warn(format!("could not write greeter environment: {error}")); }
}
