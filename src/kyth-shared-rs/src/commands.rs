...
pub const DESKTOP_ENV_KEYS: &[&str] = &[
    "DBUS_SESSION_BUS_ADDRESS", "DESKTOP_STARTUP_ID", "DISPLAY", "HOME", "LANG", "LC_ALL", "PATH",
    "SUDO_ASKPASS", "WAYLAND_DISPLAY", "XAUTHORITY", "XDG_CURRENT_DESKTOP", "XDG_RUNTIME_DIR",
];

const UNSAFE_ENV_NAMES: &[&str] = &[
    "BASH_ENV", "ENV", "GCONV_PATH", "PERL5LIB", "PERL5OPT", "RUBYLIB", "RUBYOPT",
    "JAVA_TOOL_OPTIONS", "_JAVA_OPTIONS", "JDK_JAVA_OPTIONS", "NODE_OPTIONS",
];

const UNSAFE_ENV_PREFIXES: &[&str] = &["LD_", "PYTHON", "PERL", "RUBY", "NODE_", "GEM_", "JAVA_"];

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EnvironmentPolicy { Inherit, Sanitized, Desktop }
...
impl CommandSpec {
    ...
    pub fn display_argv(&self) -> Vec<String> {
        ...
        for index in 0..displayed.len().saturating_sub(1) {
            if self.sensitive_options.iter().any(|option| option == &displayed[index]) {
...
pub fn sanitized_environment(environment: &BTreeMap<String, String>) -> BTreeMap<String, String> {
    environment.iter().filter(|(key, _)| {
        !UNSAFE_ENV_NAMES.iter().any(|unsafe_name| key.as_str() == *unsafe_name)
...
pub fn desktop_environment(environment: &BTreeMap<String, String>) -> BTreeMap<String, String> {
    environment.iter().filter(|(key, _)| DESKTOP_ENV_KEYS.iter().any(|allowed| key.as_str() == *allowed))
        .map(|(key, value)| (key.clone(), value.clone())).collect()
}

pub fn environment_for(policy: EnvironmentPolicy, environment: &BTreeMap<String, String>) -> BTreeMap<String, String> {
    match policy {
...
mod tests {
    ...
    fn env() -> BTreeMap<String, String> {
        BTreeMap::from([
            ...
            ("DBUS_SESSION_BUS_ADDRESS".into(), "unix:path=/run/user".into()),
    ...
    fn validates_argv_and_ujust_recipe_without_shell_expansion() {
        assert_eq!(ujust_command("gaming-mode").unwrap(), vec!["ujust", "gaming-mode"]);
    ...
    fn redacts_values_following_sensitive_options() {
        let mut spec = CommandSpec::new(vec!["tool".into(), "--token".into(), "secret".into(), "--mode".into(), "safe".into()]).unwrap();
        spec.sensitive_options = vec!["--token".into()];
        assert_eq!(spec.display_argv(), vec!["tool", "--token", "<redacted>", "--mode", "safe"]);
    }
}
