...
fn main() {
    ...
    let report = kyth_shared::system::windows_verify::verify(&home, std::path::Path::new("/var/home").exists());
    if json {
    ...
    if report.parity == "ok" { std::process::exit(0); }
    std::process::exit(1);
}
...
mod tests {
    ...
    fn completed_markers_are_reported_ok() {
        ...
        assert_eq!(kyth_shared::system::windows_verify::verify(home.path(), true).parity, "ok");
    }
}
