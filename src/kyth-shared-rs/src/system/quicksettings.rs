...
impl Default for QuickSettingsConfig {
    fn default() -> Self {
        Self {
            brightness: 80,
            tiles: vec!["wifi".into(), "bt".into(), "night".into(), "plane".into()],
        }
    }
}
...
fn normalize_tiles(value: Option<&toml::Value>) -> Vec<String> {
    let tiles = value
        ...
        .unwrap_or_default();
    if tiles.is_empty() {
        vec!["wifi".into(), "bt".into(), "night".into()]
    } else {
        tiles
    }
}
...
pub fn config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    ...
    if let Some(config) = std::env::var_os("XDG_CONFIG_HOME") {
        return PathBuf::from(config).join("kyth/quicksettings.toml");
    }
    PathBuf::from(std::env::var_os("HOME").unwrap_or_else(|| ".".into()))
        .join(".config/kyth/quicksettings.toml")
}

pub fn load(path: impl AsRef<Path>) -> QuickSettingsConfig {
    let Ok(raw) = std::fs::read_to_string(path) else {
        return QuickSettingsConfig::default();
    };
...
pub fn save(path: impl AsRef<Path>, config: &QuickSettingsConfig) -> std::io::Result<()> {
    ...
    let rendered = tiles
        ...
        .join(", ");
    let text =
        format!("# Kyth QuickSettings deep\nbrightness = {brightness}\ntiles = [{rendered}]\n");
