...
impl Default for GameHdr {
    fn default() -> Self { Self { peak_nits: 400, itm: false } }
}

fn clamp(config: GameHdr) -> GameHdr {
    GameHdr { peak_nits: config.peak_nits.clamp(100, 4_000), itm: config.itm }
}
...
pub fn load(path: impl AsRef<Path>) -> BTreeMap<String, GameHdr> {
    ...
    value.get("games").and_then(toml::Value::as_table).map(|games| games.iter().filter_map(|(app, value)| {
        ...
        Some((app.clone(), clamp(GameHdr {
            peak_nits: table.get("peak_nits").and_then(toml::Value::as_integer).unwrap_or(400),
            itm: table.get("itm").and_then(toml::Value::as_bool).unwrap_or(false),
        })))
...
pub fn save(path: impl AsRef<Path>, games: &BTreeMap<String, GameHdr>) -> std::io::Result<()> {
    ...
    for (app, config) in games {
        ...
        text.push_str(&format!("[games.{app:?}]\npeak_nits = {}\nitm = {}\n\n", config.peak_nits, config.itm));
    }
    crate::atomic_io::atomic_write_text(path, &text, Some(0o600))
}

pub fn for_app(app: &str, path: impl AsRef<Path>) -> Option<GameHdr> { load(path).remove(app) }
...
mod tests {
    ...
    fn clamps_and_round_trips_game_settings() {
        ...
        games.insert("123".into(), GameHdr { peak_nits: 9_000, itm: true });
        save(&path, &games).unwrap();
        assert_eq!(for_app("123", &path), Some(GameHdr { peak_nits: 4_000, itm: true }));
