...
pub fn normalise_filename(filename: &str) -> String {
    ...
    let wrapper = Regex::new(r"^(setup|install|installer|update|updater|launcher)[-_]+").expect("static wrapper prefix pattern");
    let wrapper_suffix = Regex::new(r"[-_]+(setup|install|installer|update|updater|launcher)$").expect("static wrapper suffix pattern");
...
pub fn rewrite_steam_exec(exec_line: &str) -> Option<String> {
    ...
    if let Some(capture) = game_pattern.captures(target).and_then(|capture| capture.get(1)) {
        return Some(format!("Exec=flatpak run com.valvesoftware.Steam steam://rungameid/{}", capture.as_str()));
    }
    let app_pattern = Regex::new(r"-applaunch\s+([0-9]+)").expect("static Steam applaunch pattern");
    app_pattern.captures(target).and_then(|capture| capture.get(1)).map(|capture| {
        format!("Exec=flatpak run com.valvesoftware.Steam steam://rungameid/{}", capture.as_str())
    })
}
...
fn entry_result(entry: &Value) -> CompatResult {
    CompatResult {
        status: entry.get("status").and_then(Value::as_str).unwrap_or("Works").to_string(),
        runner: entry.get("runner").and_then(Value::as_str).unwrap_or("Wine").to_string(),
        reason: entry.get("reason").and_then(Value::as_str).unwrap_or("").to_string(),
    }
}
...
pub fn check_exe(path: impl AsRef<Path>, compat: &Value) -> CompatResult {
    ...
    let filename = path.file_name().and_then(|name| name.to_str()).unwrap_or("").to_ascii_lowercase();
    if let Some(entry) = hash.as_deref().and_then(|key| entries.and_then(|entries| entries.get(key))).or_else(|| entries.and_then(|entries| entries.get(&filename))) {
        return entry_result(entry);
    }
    for marker in ["easyanticheat", "eac", "vgc", "battleye"] {
        if filename.contains(marker) {
            return CompatResult { status: "Blocked".to_string(), runner: "Anti-cheat".to_string(), reason: format!("Contains {marker} — blocked") };
        }
    }
    CompatResult { status: "Works".to_string(), runner: "Bottles".to_string(), reason: "Offline DB: best-effort Wine".to_string() }
}
...
mod tests {
    ...
    fn uses_filename_then_anticheat_fallback() {
        ...
        assert_eq!(check_exe(&blocked, &serde_json::json!({"entries":{}})).status, "Blocked");
    }

    #[test]
    fn normalizes_installer_names_and_rewrites_steam_launchers() {
        assert_eq!(normalise_filename("/tmp/Setup My.Game v1.2 x64.exe"), "my-game");
