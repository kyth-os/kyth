...
impl Default for PerfGateConfig {
    fn default() -> Self { Self { threshold: 10, enabled: true } }
}
...
pub fn config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    ...
    if std::env::var("KYTH_TEST_MODE").ok().as_deref() == Some("1") {
        if let Some(config) = std::env::var_os("XDG_CONFIG_HOME") { return PathBuf::from(config).join("kyth/perf-gate.toml"); }
    }
    PathBuf::from("/etc/kyth/perf-gate.toml")
}

pub fn load(path: impl AsRef<Path>) -> PerfGateConfig {
    let Ok(raw) = std::fs::read_to_string(path) else { return PerfGateConfig::default(); };
    let Ok(value) = raw.parse::<toml::Value>() else { return PerfGateConfig::default(); };
    PerfGateConfig {
        threshold: value.get("threshold").and_then(toml::Value::as_integer).unwrap_or(10).clamp(1, 20),
...
pub fn save(path: impl AsRef<Path>, config: PerfGateConfig) -> std::io::Result<()> {
    let config = PerfGateConfig { threshold: config.threshold.clamp(1, 20), ..config };
    let text = format!("# Kyth perf gate — offline\nthreshold = {}\nenabled = {}\n", config.threshold, config.enabled);
...
fn last_p95(ledger: impl AsRef<Path>) -> Option<f64> {
    ...
    raw.lines().rev().take(10).find_map(|line| {
        serde_json::from_str::<Value>(line).ok()?.get("p95")?.as_f64()
    })
}

pub fn check(config: PerfGateConfig, current_ms: Option<f64>, ledger: impl AsRef<Path>) -> PerfGateResult {
    if !config.enabled {
        return PerfGateResult { enabled: false, pass: true, threshold: None, last: None, current: None, delta: None };
    }
    let last = last_p95(ledger);
    let delta = last.zip(current_ms).map(|(last, current)| if last != 0.0 { (current - last) / last * 100.0 } else { 0.0 });
    PerfGateResult {
        enabled: true,
...
mod tests {
    ...
    fn clamps_config_and_reads_recent_ledger_value() {
        ...
        assert_eq!(load(&config_path), PerfGateConfig { threshold: 20, enabled: false });
    ...
    fn disabled_gate_always_passes() {
        let directory = tempdir().unwrap();
        let result = check(PerfGateConfig { threshold: 1, enabled: false }, Some(9_999.0), directory.path().join("missing"));
        assert!(result.pass);
