...
pub fn write_app_status(path: impl AsRef<Path>, state: &str, message: &str, updated: &str) -> std::io::Result<()> {
...
mod tests {
    ...
    fn renders_and_writes_firstboot_status_contract() {
        ...
        assert_eq!(app_status_content("ready", "All set", "now"), "state=ready\nmessage=All set\nupdated=now\n");
