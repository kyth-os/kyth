...
pub fn lock_path(override_path: Option<impl AsRef<Path>>) -> PathBuf {
    if let Some(path) = override_path { return path.as_ref().to_path_buf(); }
    std::env::var_os("KYTH_AKMODS_LOCK").filter(|value| !value.is_empty()).map(PathBuf::from).unwrap_or_else(|| PathBuf::from(DEFAULT_LOCK_PATH))
}

fn busy(error: Errno) -> bool { error == Errno::AGAIN || error == Errno::WOULDBLOCK }
...
pub fn acquire(path: impl AsRef<Path>, timeout: Duration) -> std::io::Result<File> {
    ...
    loop {
        match flock(&file, FlockOperation::NonBlockingLockExclusive) {
            Ok(()) => return Ok(file),
            Err(error) if busy(error) && Instant::now() < deadline => std::thread::sleep(Duration::from_millis(200)),
            Err(error) if busy(error) => return Err(std::io::Error::new(std::io::ErrorKind::TimedOut, "another NVIDIA module build is already running; wait for it to finish")),
