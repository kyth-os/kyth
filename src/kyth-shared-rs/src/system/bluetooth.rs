...
impl Default for BluetoothPreset {
    fn default() -> Self { Self { codec: "LC3".into(), latency: "low".into() } }
}
...
pub fn load(path: impl AsRef<Path>) -> BTreeMap<String, BluetoothPreset> {
    ...
    value.get("devices").and_then(toml::Value::as_table).map(|devices| devices.iter().filter_map(|(address, value)| {
        let entry = value.as_table()?;
        Some((address.clone(), BluetoothPreset {
            codec: entry.get("codec").and_then(toml::Value::as_str).unwrap_or("LC3").into(),
            latency: entry.get("latency").and_then(toml::Value::as_str).unwrap_or("low").into(),
        }))
    }).collect()).unwrap_or_default()
}

pub fn save(path: impl AsRef<Path>, devices: &BTreeMap<String, BluetoothPreset>) -> std::io::Result<()> {
    let mut text = String::from("# Kyth Bluetooth LE Audio per-device\n");
    for (address, preset) in devices {
        text.push_str(&format!("[devices.{address:?}]\ncodec = {:?}\nlatency = {:?}\n\n", preset.codec, preset.latency));
    }
...
mod tests {
    ...
    fn loads_defaults_for_partial_device_entries() {
        ...
        assert_eq!(load(&path)["aa-bb"], BluetoothPreset { codec: "opus".into(), latency: "low".into() });
    }
    ...
    fn saves_sorted_device_presets() {
        ...
        assert!(std::fs::read_to_string(path).unwrap().contains("[devices.\"aa-bb\"]"));
    }
}
