...
pub fn parse_snapper_rows(output: &str) -> Vec<SnapshotRow> {
    ...
    data.get("snapshots")
        ...
        .map(|snapshots| snapshots.iter().map(|snapshot| SnapshotRow {
...
pub fn parse_bootc_rows(data: &Value) -> Vec<SnapshotRow> {
    ["booted", "rollback", "staged"].into_iter().filter_map(|section| {
        ...
        Some(SnapshotRow {
            id,
            timestamp: String::new(),
            row_type: if section == "booted" { "deployment" } else { "rollback" }.to_string(),
...
mod tests {
    ...
    fn serializes_supplied_rows_with_python_wire_keys() {
        let rows = [SnapshotRow { id: "7".into(), timestamp: "today".into(), row_type: "snapshot".into(), description: "before update".into(), healthy: None }];
    ...
    fn parses_snapper_rows_without_running_snapper() {
        let rows = parse_snapper_rows(r#"{"snapshots":[{"number":7,"date":"2026-08-30","description":"before update"}]}"#);
        assert_eq!(rows, vec![SnapshotRow { id: "7".into(), timestamp: "2026-08-30".into(), row_type: "snapshot".into(), description: "before update".into(), healthy: None }]);
