...
pub fn plymouth_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    system_path("plymouth.toml", path)
}
pub fn load_plymouth(path: impl AsRef<Path>) -> PlymouthConfig {
    let Ok(raw) = std::fs::read_to_string(path) else {
        return PlymouthConfig::default();
    };
...
pub fn save_plymouth(path: impl AsRef<Path>, config: &PlymouthConfig) -> std::io::Result<()> {
    crate::atomic_io::atomic_write_text(
        path,
        &format!(
            "# Kyth Plymouth theme preset\ntheme = {}\nduration = {}\n",
            quote(&config.theme),
            config.duration
        ),
        Some(0o600),
...
pub fn ensure_shader_cache_dir(
    appid: &str,
    driver_version: &str,
    glsl_text: &str,
    root: impl AsRef<Path>,
) -> std::io::Result<PathBuf> {
    let path = shader_cache_dir(appid, driver_version, glsl_text, root);
    std::fs::create_dir_all(&path)?;
    Ok(path)
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PolkitRules {
    pub flatpak: bool,
    pub btrfs: bool,
}
impl Default for PolkitRules {
    fn default() -> Self {
        Self {
            flatpak: true,
            btrfs: true,
...
pub fn polkit_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    system_path("polkit.toml", path)
}
pub fn load_polkit(path: impl AsRef<Path>) -> PolkitRules {
    let Ok(raw) = std::fs::read_to_string(path) else {
        return PolkitRules::default();
    };
    let Ok(value) = raw.parse::<toml::Value>() else {
        return PolkitRules::default();
    };
    let Some(rules) = value.get("rules").and_then(toml::Value::as_table) else {
        return PolkitRules::default();
    };
    PolkitRules {
        flatpak: rules
...
pub fn save_polkit(path: impl AsRef<Path>, rules: &PolkitRules) -> std::io::Result<()> {
    crate::atomic_io::atomic_write_text(
        path,
        &format!(
            "# Kyth polkit presets\n[rules]\nflatpak = {}\nbtrfs = {}\n",
            rules.flatpak, rules.btrfs
        ),
        Some(0o600),
    )
}
pub fn generate_polkit_rules(rules: &PolkitRules) -> String {
    ...
    if rules.flatpak {
        lines.push("polkit.addRule(function(a,s){if(a.id.indexOf(\"org.freedesktop.Flatpak\")==0 && s.isInGroup(\"wheel\"))return polkit.Result.YES;});".into());
    }
    if rules.btrfs {
        lines.push("polkit.addRule(function(a,s){if(a.id==\"org.freedesktop.UDisks2.modify-device\" && s.isInGroup(\"wheel\"))return polkit.Result.YES;});".into());
    }
    format!("{}\n", lines.join("\n"))
}
...
pub fn scx_config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    ...
    if let Some(xdg) = std::env::var_os("XDG_CONFIG_HOME") {
        return PathBuf::from(xdg).join("kyth/scx.toml");
    }
        ...
        .map(|games| {
            games
                ...
                .filter_map(|(app, value)| {
                    let scx = value
                        .as_table()
                        .and_then(|table| table.get("scx"))
                        .and_then(toml::Value::as_str)
                        .or_else(|| value.as_str())?;
                    matches!(scx, "scx_rusty" | "scx_bpfland" | "scx_lavd" | "none")
                        .then(|| (app.clone(), scx.to_string()))
                })
                .collect()
        })
        .unwrap_or_default()
...
pub fn save_scx(path: impl AsRef<Path>, games: &BTreeMap<String, String>) -> std::io::Result<()> {
    ...
    for (app, scx) in games {
        lines.push(format!("[games.{}]", quote(app)));
        lines.push(format!("scx = {}", quote(scx)));
        lines.push(String::new());
    }
    crate::atomic_io::atomic_write_text(path, &format!("{}\n", lines.join("\n")), Some(0o600))
}
pub fn scx_for_app(app: &str, path: impl AsRef<Path>) -> Option<String> {
    load_scx(path).remove(app)
}
...
mod tests {
    ...
    fn scx_config_path_prefers_explicit_then_xdg_then_home() {
        assert_eq!(
            scx_config_path(Some("/tmp/explicit.toml")),
    ...
    fn multi_game_scx_selects_lexicographically_first_app() {
        ...
        let games = BTreeMap::from([
            ("zelda".into(), "scx_lavd".into()),
            ("ananicy".into(), "scx_bpfland".into()),
        ]);
        save_scx(&path, &games).unwrap();
        let loaded = load_scx(&path);
        assert_eq!(
            loaded.iter().next(),
            Some((&"ananicy".to_string(), &"scx_bpfland".to_string()))
        );
    }
}
