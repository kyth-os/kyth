...
use super::boot_health::{clear_quarantine, mark_healthy, note_rollback_attempted, read_state, record_failure, record_staged, BootHealthState};
...
impl UpdateCoordinator {
    ...
    pub fn record_staged(&self, digest: &str, rollout_ring: &str, now: i64) -> std::io::Result<BootHealthState> {
        self.transaction(|state| record_staged(&state, digest, rollout_ring, now))
    }

    pub fn record_failure(&self, digest: &str, boot_id: &str, reason: &str, threshold: i64, now: i64) -> std::io::Result<BootHealthState> {
    ...
    pub fn note_rollback_attempted(&self, digest: &str, error: Option<&str>, now: i64) -> std::io::Result<BootHealthState> {
...
mod tests {
    ...
    fn convenience_transitions_use_the_same_locked_writer() {
        ...
        let path = directory.path().join("boot-health.json");
        let coordinator = UpdateCoordinator::new(&path);
        coordinator.record_staged("sha256:new", "testing", 1).unwrap();
        let state = coordinator.record_failure("sha256:new", "boot-1", "display", 1, 2).unwrap();
        assert_eq!(state.status, "quarantined");
        coordinator.note_rollback_attempted("sha256:new", Some("failed"), 3).unwrap();
