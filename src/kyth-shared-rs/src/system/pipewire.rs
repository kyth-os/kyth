...
pub fn apply_pipewire_quantum(preset: &str, dry_run: bool) -> (bool, String) {
    ...
    let xdg = std::env::var("XDG_CONFIG_HOME").unwrap_or_else(|_| format!("{}/.config", std::env::var("HOME").unwrap_or_default()));
    ...
    let content = format!("# kyth quantum {}\ncontext.properties = {{\n  default.clock.quantum = {}\n}}\n", preset, q);
...
mod tests {
    ...
    fn presets_sorted() {
        ...
        assert_eq!(v, { let mut s=v.clone(); s.sort(); s});
    }
    #[test]
    fn dry_run() {
