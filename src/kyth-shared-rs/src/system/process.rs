...
pub fn run_bounded_with_input(argv: &[String], input: &[u8], timeout: Duration) -> io::Result<Output> {
    let (program, args) = argv
        .split_first()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "command must not be empty"))?;
    let mut command = Command::new(program);
...
fn redact_sensitive_line(line: &str) -> String {
    ...
    for marker in MARKERS {
        ...
        while let Some(relative) = lower[search_from..].find(marker) {
            ...
            if line.as_bytes().get(delimiter).is_some_and(|byte| matches!(byte, b'"' | b'\'')) {
                delimiter += 1;
            }
            ...
            let quote = line.as_bytes().get(value_start).copied().filter(|byte| matches!(byte, b'"' | b'\''));
            if quote.is_some() {
                value_start += 1;
            }
            ...
            if let Some(quote) = quote {
                ...
                while value_end < line.len()
                    ...
                    && !matches!(line.as_bytes()[value_end], b',' | b';' | b'&' | b'"' | b'\'' | b']' | b'}')
                {
                    value_end += 1;
                }
            }
