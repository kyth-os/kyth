...
fn allowed_device_path(path: &Path) -> bool {
    if !path.is_absolute() || !path.starts_with("/dev") { return false; }
    if path.components().any(|component| matches!(component, Component::ParentDir)) { return false; }
    path.to_string_lossy().bytes().all(|byte| byte.is_ascii_alphanumeric() || b"._/+:-".contains(&byte))
}
...
pub fn normalize_device_path(name: Option<&str>) -> Option<PathBuf> {
    ...
    let requested = if name.starts_with("/dev/") { PathBuf::from(name) } else { PathBuf::from("/dev").join(name) };
...
mod tests {
    ...
    fn accepts_device_names_and_rejects_path_escape() {
        assert_eq!(normalize_device_path(Some("sda")), Some(PathBuf::from("/dev/sda")));
        assert_eq!(normalize_device_path(Some("/dev/nvme0n1")), Some(PathBuf::from("/dev/nvme0n1")));
