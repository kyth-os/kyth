...
impl Profile {
    pub fn as_str(self) -> &'static str {
        match self { Self::Balanced => "balanced", Self::Gaming => "gaming" }
    }
}
...
pub fn load_profile(path: impl AsRef<Path>) -> Profile {
    let Ok(raw) = std::fs::read_to_string(path) else { return Profile::Balanced; };
    let Ok(value) = raw.parse::<toml::Value>() else { return Profile::Balanced; };
    profile_from_str(value.get("profile").and_then(toml::Value::as_str))
}

pub fn save_profile(path: impl AsRef<Path>, comment: &str, profile: Profile) -> std::io::Result<()> {
    let text = format!("# {comment} — offline\nprofile = \"{}\"\n", profile.as_str());
    crate::atomic_io::atomic_write_text(path, &text, Some(0o600))
}

pub fn status_from_conf(path: impl AsRef<Path>) -> Profile {
    if path.as_ref().is_file() { Profile::Gaming } else { Profile::Balanced }
}

pub fn config_path(path: Option<impl AsRef<Path>>, default_path: impl AsRef<Path>, test_filename: &str) -> PathBuf {
