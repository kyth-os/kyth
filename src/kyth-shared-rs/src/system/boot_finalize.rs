...
pub fn prepare_boot() -> Result<(), String> {
    let remounted = successful("mount", &["-o", "remount,bind,rw", "/boot"], Duration::from_secs(15))
        || successful("mount", &["-o", "remount,rw", "/boot"], Duration::from_secs(15));
    if !remounted {
        return Err("could not remount /boot read-write".to_string());
    }
    let sysroot_boot = Path::new("/sysroot/boot");
    if sysroot_boot.is_dir() && !successful("findmnt", &["-n", "/sysroot/boot"], Duration::from_secs(5)) {
        let _ = run("mount", &["--bind", "/boot", "/sysroot/boot"], Duration::from_secs(15));
    }
    Ok(())
}
...
pub fn finalize_staged(reboot: bool) -> Result<String, String> {
    ...
    let output = run("/usr/bin/ostree", &["admin", "finalize-staged"], Duration::from_secs(120))?;
    ...
    if reboot {
        ...
        if !reboot_result.status.success() {
            return Err(String::from_utf8_lossy(&reboot_result.stderr).trim().to_string());
        }
    }
