...
pub fn master_config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    config_path(path, "/etc/kyth/gaming-performance.toml", "gaming-performance.toml")
}
...
pub fn thermal_high(root: impl AsRef<Path>, threshold_c: i64) -> bool {
    ...
    entries.flatten().filter(|entry| entry.file_name().to_string_lossy().starts_with("thermal_zone")).any(|entry| {
        std::fs::read_to_string(entry.path().join("temp")).ok().and_then(|value| value.trim().parse::<i64>().ok()).is_some_and(|temp| temp > threshold_c * 1000)
    })
}

pub fn battery_low(root: impl AsRef<Path>, threshold_pct: i64) -> bool {
    let Ok(entries) = root.as_ref().read_dir() else { return false; };
    entries.flatten().filter(|entry| entry.file_name().to_string_lossy().starts_with("BAT")).any(|entry| {
        let capacity = std::fs::read_to_string(entry.path().join("capacity")).ok().and_then(|value| value.trim().parse::<i64>().ok());
        let status = std::fs::read_to_string(entry.path().join("status")).ok().map(|value| value.trim().to_ascii_lowercase());
        capacity.is_some_and(|capacity| capacity < threshold_pct) && status.is_some_and(|status| status == "discharging" || status == "not charging")
    })
}

pub fn effective_gaming(profile: Profile, thermal: bool, battery: bool) -> (Profile, &'static str) {
    if profile != Profile::Gaming { return (Profile::Balanced, "balanced profile selected"); }
    if thermal { return (Profile::Balanced, "thermal limit reached"); }
    if battery { return (Profile::Balanced, "battery is low while discharging"); }
