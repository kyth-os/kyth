...
impl Metacopy {
    pub fn as_str(self) -> &'static str { match self { Self::Auto => "auto", Self::On => "on", Self::Off => "off" } }
}
...
pub fn config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    ...
    if std::env::var("KYTH_TEST_MODE").ok().as_deref() == Some("1") {
        if let Some(config) = std::env::var_os("XDG_CONFIG_HOME") { return PathBuf::from(config).join("kyth/podman-overlay.toml"); }
    }
    PathBuf::from("/etc/kyth/podman-overlay.toml")
}

pub fn load(path: impl AsRef<Path>) -> Metacopy {
    let Ok(raw) = std::fs::read_to_string(path) else { return Metacopy::Auto; };
    let Ok(value) = raw.parse::<toml::Value>() else { return Metacopy::Auto; };
    parse(value.get("metacopy").and_then(toml::Value::as_str))
}

pub fn save(path: impl AsRef<Path>, value: Metacopy) -> std::io::Result<()> {
    crate::atomic_io::atomic_write_text(path, &format!("# Kyth overlay — offline\nmetacopy = {:?}\n", value.as_str()), Some(0o600))
}

pub fn resolve(value: Metacopy, on_btrfs: bool) -> Metacopy { if value == Metacopy::Auto { if on_btrfs { Metacopy::On } else { Metacopy::Off } } else { value } }

pub fn generate(value: Metacopy, on_btrfs: bool, destination: impl AsRef<Path>) -> std::io::Result<Option<PathBuf>> {
...
mod tests {
    ...
    fn renders_and_removes_drop_in() {
        ...
        assert!(std::fs::read_to_string(&path).unwrap().contains("metacopy=on"));
