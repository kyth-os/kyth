...
pub struct ShaderTmpfsConfig { pub enabled: bool, pub size: String }

impl Default for ShaderTmpfsConfig { fn default() -> Self { Self { enabled: false, size: "2G".into() } } }

fn normalize(config: ShaderTmpfsConfig) -> ShaderTmpfsConfig {
    ShaderTmpfsConfig { size: matches!(config.size.as_str(), "1G" | "2G" | "4G").then_some(config.size).unwrap_or_else(|| "2G".into()), ..config }
}

pub fn config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    if let Some(path) = path { return path.as_ref().to_path_buf(); }
    if std::env::var("KYTH_TEST_MODE").ok().as_deref() == Some("1") {
        if let Some(config) = std::env::var_os("XDG_CONFIG_HOME") { return PathBuf::from(config).join("kyth/shader-tmpfs.toml"); }
    }
    PathBuf::from("/etc/kyth/shader-tmpfs.toml")
}

pub fn load(path: impl AsRef<Path>) -> ShaderTmpfsConfig {
    let Ok(raw) = std::fs::read_to_string(path) else { return ShaderTmpfsConfig::default(); };
    let Ok(value) = raw.parse::<toml::Value>() else { return ShaderTmpfsConfig::default(); };
    normalize(ShaderTmpfsConfig { enabled: value.get("enabled").and_then(toml::Value::as_bool).unwrap_or(false), size: value.get("size").and_then(toml::Value::as_str).unwrap_or("2G").into() })
}

pub fn save(path: impl AsRef<Path>, config: &ShaderTmpfsConfig) -> std::io::Result<()> {
    let config = normalize(config.clone());
    crate::atomic_io::atomic_write_text(path, &format!("# Kyth shader tmpfs — offline\nenabled = {}\nsize = {:?}\n", config.enabled, config.size), Some(0o600))
}

pub fn generate(config: &ShaderTmpfsConfig, tmpfiles: impl AsRef<Path>, service: impl AsRef<Path>) -> std::io::Result<Option<PathBuf>> {
    let config = normalize(config.clone());
    ...
    crate::atomic_io::atomic_write_text(tmpfiles, "# Kyth shader tmpfs — generated\nd /run/kyth-shader 0755 - - -\n", Some(0o644))?;
...
mod tests {
    ...
    fn clamps_size_and_reversibly_renders_units() {
        ...
        save(&config_path, &ShaderTmpfsConfig { enabled: true, size: "8G".into() }).unwrap();
