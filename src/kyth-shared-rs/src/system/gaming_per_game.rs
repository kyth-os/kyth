...
const KNOWN_PROFILES: [&str; 7] = ["low-latency", "balanced", "battery", "quality", "hdr", "sharp", "latency"];
...
impl Default for GameProfile {
    fn default() -> Self {
        Self { profile: "balanced".to_string(), hdr: false }
    }
}
...
pub fn per_game_config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    ...
    PathBuf::from(std::env::var_os("HOME").unwrap_or_else(|| ".".into())).join(".config/kyth/gaming-per-game.toml")
}
...
pub fn load_per_game_config(path: impl AsRef<Path>) -> BTreeMap<String, GameProfile> {
    ...
    let Some(games) = value.get("games").and_then(toml::Value::as_table) else { return BTreeMap::new() };
    games
        .iter()
        .filter_map(|(appid, entry)| {
            let entry = entry.as_table()?;
            let mut profile = entry.get("profile").and_then(toml::Value::as_str).unwrap_or("balanced").to_string();
            if !KNOWN_PROFILES.contains(&profile.as_str()) {
                profile = "balanced".to_string();
            }
            let hdr = entry.get("hdr").and_then(toml::Value::as_bool).unwrap_or(false);
...
pub fn save_per_game_config(games: &BTreeMap<String, GameProfile>, path: impl AsRef<Path>) -> std::io::Result<()> {
...
pub fn set_profile_for_appid(appid: &str, profile: &str, hdr: bool, path: impl AsRef<Path>) -> std::io::Result<()> {
    let mut games = load_per_game_config(&path);
    games.insert(appid.to_string(), GameProfile { profile: profile.to_string(), hdr });
...
mod tests {
    ...
    fn round_trips_a_saved_profile() {
        ...
        assert_eq!(loaded, GameProfile { profile: "hdr".to_string(), hdr: true });
    }
    ...
    fn setting_one_appid_does_not_disturb_another() {
        ...
        assert_eq!(games["1"], GameProfile { profile: "quality".to_string(), hdr: false });
        assert_eq!(games["2"], GameProfile { profile: "latency".to_string(), hdr: true });
    }

    #[test]
    fn path_prefers_explicit_then_xdg_config_home() {
        assert_eq!(per_game_config_path(Some("/tmp/custom.toml")), PathBuf::from("/tmp/custom.toml"));
    }
}
