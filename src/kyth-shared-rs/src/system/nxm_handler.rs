...
pub fn handle(url: Option<&str>, home: &Path) -> i32 {
    ...
    if vortex_bottle_dir(home).is_dir() && bottles_cli_available() {
        let status = crate::system::process::run_bounded(
            &[
...
mod tests {
    ...
    fn missing_url_is_usage_error() {
        ...
        assert_eq!(handle(None, dir.path()), 1);
        assert_eq!(handle(Some(""), dir.path()), 1);
    }

    #[test]
    fn without_vortex_bottle_informs_and_succeeds() {
        // No bottle dir: must not attempt bottles-cli, must exit 0.
        let dir = tempfile::tempdir().unwrap();
        assert_eq!(handle(Some("nxm://example/mods/1/files/2"), dir.path()), 0);
    }
