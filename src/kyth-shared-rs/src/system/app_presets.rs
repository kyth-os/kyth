...
impl Default for AppPreset {
    fn default() -> Self {
        Self { cpu_weight: 100, memory_max: "80%".to_string(), latency: "balanced".to_string() }
    }
}
...
pub fn config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    ...
    PathBuf::from(std::env::var_os("HOME").unwrap_or_else(|| ".".into())).join(".config/kyth/app-presets.toml")
}
...
pub fn load(path: impl AsRef<Path>) -> BTreeMap<String, AppPreset> {
    ...
    let Some(apps) = value.get("apps").and_then(toml::Value::as_table) else { return BTreeMap::new(); };
    apps.iter().filter_map(|(name, value)| {
        let entry = value.as_table()?;
        Some((name.clone(), AppPreset {
            cpu_weight: entry.get("cpu_weight").and_then(toml::Value::as_integer).unwrap_or(100),
            memory_max: entry.get("memory_max").and_then(toml::Value::as_str).unwrap_or("80%").to_string(),
            latency: entry.get("latency").and_then(toml::Value::as_str).unwrap_or("balanced").to_string(),
        }))
    }).collect()
}
...
mod tests {
    ...
    fn loads_nested_app_presets_and_defaults_missing_fields() {
        ...
        let apps = load(&path);
        assert_eq!(apps["steam"], AppPreset { cpu_weight: 200, memory_max: "80%".into(), latency: "gaming".into() });
