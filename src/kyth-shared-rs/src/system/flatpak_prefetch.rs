...
pub struct FlatpakPrefetchConfig { pub enabled: bool, pub time: String }

impl Default for FlatpakPrefetchConfig { fn default() -> Self { Self { enabled: false, time: "02:00".into() } } }

fn normalize_time(value: &str) -> String { if value.contains(':') && value.len() <= 5 { value.into() } else { "02:00".into() } }

pub fn config_path(path: Option<impl AsRef<Path>>) -> PathBuf {
    if let Some(path) = path { return path.as_ref().to_path_buf(); }
    if std::env::var("KYTH_TEST_MODE").ok().as_deref() == Some("1") {
        if let Some(config) = std::env::var_os("XDG_CONFIG_HOME") { return PathBuf::from(config).join("kyth/flatpak-prefetch.toml"); }
    }
    PathBuf::from("/etc/kyth/flatpak-prefetch.toml")
}

pub fn load(path: impl AsRef<Path>) -> FlatpakPrefetchConfig {
    let Ok(raw) = std::fs::read_to_string(path) else { return FlatpakPrefetchConfig::default(); };
    let Ok(value) = raw.parse::<toml::Value>() else { return FlatpakPrefetchConfig::default(); };
    FlatpakPrefetchConfig { enabled: value.get("enabled").and_then(toml::Value::as_bool).unwrap_or(false), time: normalize_time(value.get("time").and_then(toml::Value::as_str).unwrap_or("02:00")) }
}

pub fn save(path: impl AsRef<Path>, config: &FlatpakPrefetchConfig) -> std::io::Result<()> {
    crate::atomic_io::atomic_write_text(path, &format!("# Kyth flatpak prefetch — offline\nenabled = {}\ntime = {:?}\n", config.enabled, normalize_time(&config.time)), Some(0o600))
}
...
mod tests {
    ...
    fn defaults_and_normalizes_schedule() {
        ...
        assert_eq!(load(&path), FlatpakPrefetchConfig { enabled: true, time: "02:00".into() });
        save(&path, &FlatpakPrefetchConfig { enabled: true, time: "23:30".into() }).unwrap();
