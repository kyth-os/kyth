fn main() -> std::process::ExitCode {
    ...
    let Some(operation) = args.next() else {
        eprintln!("usage: kyth-bootc-guard <status|status-json|check|switch-latest|switch-testing|switch-latest-cachy|switch-testing-cachy>");
    ...
    let result = match operation.as_str() {
        ...
        "check" => kyth_shared::system::bootc_guard::check(),
